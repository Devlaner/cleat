import { cvssToSeverity, type Severity } from "@/lib/severity";
import { Rng } from "./rng";
import {
  REPO_NAMES, LANGUAGES, TOPICS, SECRET_PROVIDERS, SECRET_FILES, PACKAGES,
  VULN_TITLES, CWES, CODE_RULES, ACTIONS, WORKFLOW_FILES, FIRST_NAMES, LAST_NAMES,
  TEAMS, OAUTH_APPS, WEBHOOK_EVENTS, PAT_SCOPES,
} from "./catalog";
import type {
  Account, Dataset, Repo, SecretFinding, Vulnerability, CodeScanAlert, WorkflowAudit,
  SupplyChainIncident, Artifact, CacheEntry, PackageEntry, Usage, Member, InstalledApp,
  Webhook, AccessKey, Pat, ActivityEvent, PostureTrendPoint, ScorecardCheck, Visibility,
} from "./types";
...
function scorecardFor(repo: Omit<Repo, "scorecard" | "hygieneScore">, r: Rng): ScorecardCheck[] {
  ...
  return [
    { id: "branch-protection", name: "Branch-Protection", score: repo.branchProtected ? r.int(7, 10) : r.int(0, 3), reason: repo.branchProtected ? "default branch is protected" : "no protection on default branch" },
    { id: "code-review", name: "Code-Review", score: repo.branchProtected ? r.int(6, 10) : r.int(2, 6), reason: "reviews on recent changes" },
    { id: "pinned-dependencies", name: "Pinned-Dependencies", score: clamp(10 - unpinned, 0, 10), reason: unpinned > 0 ? `${unpinned} unpinned dependencies` : "dependencies are pinned" },
    { id: "token-permissions", name: "Token-Permissions", score: r.int(3, 10), reason: "GITHUB_TOKEN permission scope" },
    { id: "dangerous-workflow", name: "Dangerous-Workflow", score: r.int(5, 10), reason: "no dangerous workflow patterns" },
    { id: "maintained", name: "Maintained", score: repo.archived ? r.int(0, 2) : r.int(5, 10), reason: repo.archived ? "repository is archived" : "active commits in last 90 days" },
    { id: "sast", name: "SAST", score: repo.hasCI ? r.int(4, 10) : r.int(0, 4), reason: repo.hasCI ? "SAST configured in CI" : "no SAST tooling detected" },
    { id: "vulnerabilities", name: "Vulnerabilities", score: clamp(10 - repo.openVulns, 0, 10), reason: repo.openVulns > 0 ? `${repo.openVulns} open advisories` : "no open advisories" },
  ];
}
...
function genRepos(r: Rng, account: Account): Repo[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const visibility: Visibility = personal
      ? r.weighted<Visibility>([["public", 6], ["private", 4]])
      : r.weighted<Visibility>([["private", 6], ["internal", 2], ["public", 2]]);
    ...
    const partial = {
      ...
      stars: r.weighted([[r.int(0, 40), 6], [r.int(40, 800), 3], [r.int(800, 12000), 1]]),
      ...
      hasCI,
      sizeMb: r.weighted([[r.int(1, 60), 5], [r.int(60, 500), 3], [r.int(500, 2400), 1]]),
      lastPushedAt: r.daysAgo(0, archived ? 400 : 180),
      archived,
      openVulns: 0,
      openSecrets: 0,
      openCodeAlerts: 0,
      staleBranches: r.weighted([[0, 3], [r.int(1, 5), 4], [r.int(5, 16), 2]]),
      openPRs: r.int(0, 18),
...
function genSecrets(r: Rng, repos: Repo[], account: Account): SecretFinding[] {
  ...
  for (let i = 0; i < target; i++) {
    ...
    const validity = r.weighted<SecretFinding["validity"]>([["active", 4], ["revoked", 5], ["unknown", 2]]);
    const severity: Severity = validity === "active" ? r.weighted<Severity>([["critical", 6], ["high", 4]]) : r.weighted<Severity>([["high", 3], ["medium", 4], ["low", 3]]);
    out.push({
...
function genVulns(r: Rng, repos: Repo[], account: Account): Vulnerability[] {
  ...
  for (let i = 0; i < target; i++) {
    ...
    const cvss = r.weighted([[r.range(9, 10, 1), 2], [r.range(7, 8.9, 1), 4], [r.range(4, 6.9, 1), 5], [r.range(0.1, 3.9, 1), 2]]);
    ...
    out.push({
      ...
      cvss,
      severity,
      epss: r.weighted([[r.range(0.5, 0.97, 3), 2], [r.range(0.05, 0.5, 3), 4], [r.range(0.001, 0.05, 3), 5]]),
      kev,
      reachable: r.weighted<Vulnerability["reachable"]>([["reachable", 4], ["not-reachable", 4], ["unknown", 2]]),
...
function genCodeAlerts(r: Rng, repos: Repo[], account: Account): CodeScanAlert[] {
  ...
  for (let i = 0; i < target; i++) {
    ...
    const status = r.weighted<CodeScanAlert["status"]>([["open", 6], ["fixed", 3], ["dismissed", 1]]);
    out.push({
...
function genWorkflows(r: Rng, repos: Repo[], account: Account): WorkflowAudit[] {
  ...
  for (const repo of ciRepos) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      const permissions = r.weighted<"broad" | "scoped">([["broad", 3], ["scoped", 5]]);
      const usesOidc = r.chance(0.4);
      const risk = clamp(unpinned * 14 + (permissions === "broad" ? 28 : 0) + (usesOidc ? 0 : 12), 0, 100);
      out.push({
...
function genIncidents(r: Rng, account: Account): SupplyChainIncident[] {
  const base: Omit<SupplyChainIncident, "id" | "affectedReposCount" | "status">[] = [
    { title: "tj-actions/changed-files compromised", cve: "CVE-2025-30066", affectedAction: "tj-actions/changed-files", severity: "critical", summary: "Malicious code injected via retagged versions dumped CI secrets into build logs. Pin to a full commit SHA.", publishedAt: r.daysAgo(40, 70) },
    { title: "Shai-Hulud npm worm propagation", cve: "GHSA-shai-hulud", affectedAction: "npm: multiple packages", severity: "high", summary: "Self-propagating npm compromise reusing token-extraction techniques. Audit recently installed packages.", publishedAt: r.daysAgo(10, 30) },
    { title: "reviewdog action token exposure", cve: "CVE-2025-30154", affectedAction: "reviewdog/action-setup", severity: "high", summary: "A compromised action exposed GITHUB_TOKEN in logs for affected workflows.", publishedAt: r.daysAgo(35, 65) },
    { title: "Unpinned third-party action drift", cve: "No CVE", affectedAction: "multiple", severity: "medium", summary: "Several workflows reference mutable tags (@v4, @main) instead of immutable commit SHAs.", publishedAt: r.daysAgo(2, 14) },
  ];
  return base.map((b, i) => ({
    ...b,
    id: `${account.id}_inc_${i}`,
    affectedReposCount: r.int(1, Math.max(2, Math.round(account.repoCount * 0.15))),
    status: r.weighted<SupplyChainIncident["status"]>([["action-required", 4], ["monitoring", 3], ["resolved", 3]]),
  }));
}

function genArtifacts(r: Rng, repos: Repo[], account: Account): Artifact[] {
  const target = clamp(account.repoCount * 2, 10, 120);
  const names = ["build-output", "dist", "coverage-report", "test-results", "docker-image", "playwright-report", "bundle-stats", "sourcemaps", "android-apk", "ios-ipa"];
  const out: Artifact[] = [];
  for (let i = 0; i < target; i++) {
    const repo = r.pick(repos);
    ...
    out.push({
      ...
      sizeMb: r.weighted([[r.range(1, 50, 1), 5], [r.range(50, 400, 1), 3], [r.range(400, 2200, 1), 2]]),
      createdAt: created,
      expiresAt: r.daysAhead(0, 90),
      workflowRun: `#${r.int(120, 9800)}`,
      downloads: r.weighted([[0, 6], [r.int(1, 12), 3], [r.int(12, 200), 1]]),
    });
  }
  return out;
}
...
function genPackages(r: Rng, repos: Repo[], account: Account): PackageEntry[] {
  ...
  for (let i = 0; i < target; i++) {
    ...
    out.push({
      ...
      untaggedLayers: r.weighted([[0, 3], [r.int(1, 40), 4], [r.int(40, 200), 2]]),
...
function genMembers(r: Rng, account: Account): Member[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const role = i === 0 ? "owner" : r.weighted<Member["role"]>([["owner", 1], ["admin", 2], ["member", 12]]);
...
function genApps(r: Rng, account: Account): InstalledApp[] {
  ...
  return chosen.map((a, i) => {
    ...
    return {
      ...
      risk: risky ? r.weighted<Severity>([["high", 3], ["medium", 3], ["critical", 1]]) : r.weighted<Severity>([["low", 3], ["info", 3], ["medium", 1]]),
    };
  });
}
...
function genWebhooks(r: Rng, repos: Repo[], account: Account): Webhook[] {
  ...
  const out: Webhook[] = [];
  const hosts = ["hooks.slack.com", "api.example.com", "ci.internal.dev", "deploy.acme.io", "n8n.self-hosted.net", "discord.com/api/webhooks"];
  for (let i = 0; i < target; i++) {
...
function genPats(r: Rng, account: Account): Pat[] {
  ...
  const out: Pat[] = [];
  for (let i = 0; i < count; i++) {
    const kind = r.weighted<Pat["kind"]>([["classic", 5], ["fine-grained", 4]]);
    const scopes = kind === "classic" ? r.pickN(PAT_SCOPES, r.int(1, 5)) : r.pickN(["contents:read", "contents:write", "metadata:read", "pull_requests:write", "actions:read"], r.int(1, 3));
    const expires = r.weighted<"none" | "future" | "past">([["none", 3], ["future", 4], ["past", 2]]);
    out.push({
      ...
      expiresAt: expires === "none" ? null : expires === "future" ? r.daysAhead(1, 120) : r.daysAgo(1, 60),
    });
  }
  return out;
}
...
function genEvents(r: Rng, repos: Repo[], account: Account): ActivityEvent[] {
  const templates: { type: string; category: ActivityEvent["category"]; severity: Severity; msg: (repo: string, actor: string) => string }[] = [
    { type: "secret.detected", category: "security", severity: "critical", msg: (repo) => `Active AWS secret detected in ${repo}` },
    { type: "branch_protection.disabled", category: "access", severity: "high", msg: (repo, a) => `${a} disabled branch protection on ${repo}` },
    { type: "repo.made_public", category: "access", severity: "high", msg: (repo, a) => `${a} changed ${repo} visibility to public` },
    { type: "member.added_admin", category: "access", severity: "high", msg: (_r, a) => `${a} was granted admin on the organization` },
    { type: "oauth_app.created", category: "access", severity: "medium", msg: (_r, a) => `${a} authorized a new OAuth app` },
    { type: "action.unpinned", category: "supply-chain", severity: "medium", msg: (repo) => `Unpinned third-party action found in ${repo}` },
    { type: "force_push.main", category: "maintenance", severity: "medium", msg: (repo, a) => `${a} force-pushed to main on ${repo}` },
    { type: "vuln.kev", category: "security", severity: "critical", msg: (repo) => `Actively exploited (KEV) advisory affects ${repo}` },
    { type: "deploy_key.added", category: "access", severity: "low", msg: (repo, a) => `${a} added a deploy key to ${repo}` },
    { type: "fix_pr.merged", category: "maintenance", severity: "info", msg: (repo, a) => `${a} merged a Dependabot fix PR in ${repo}` },
    { type: "2fa.disabled", category: "access", severity: "high", msg: (_r, a) => `${a} disabled two-factor authentication` },
    { type: "webhook.failing", category: "maintenance", severity: "low", msg: (repo) => `Webhook deliveries failing for ${repo}` },
  ];
