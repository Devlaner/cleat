...
export const CODE_RULES: { rule: string; ruleId: string; desc: string }[] = [
  {
    rule: "SQL query built from user-controlled sources",
    ruleId: "js/sql-injection",
    desc: "Untrusted input flows into a SQL query without sanitization.",
  },
  {
    rule: "Reflected cross-site scripting",
    ruleId: "js/reflected-xss",
    desc: "User input is written to the page without escaping.",
  },
  {
    rule: "Hard-coded credentials",
    ruleId: "js/hardcoded-credentials",
    desc: "Credentials are embedded directly in source code.",
  },
  {
    rule: "Uncontrolled command line",
    ruleId: "js/command-line-injection",
    desc: "A shell command is built from untrusted input.",
  },
  {
    rule: "Insecure randomness",
    ruleId: "js/insecure-randomness",
    desc: "A weak PRNG is used in a security-sensitive context.",
  },
  {
    rule: "Missing rate limiting",
    ruleId: "js/missing-rate-limiting",
    desc: "An expensive route has no rate limiting.",
  },
  {
    rule: "Clear-text logging of sensitive data",
    ruleId: "py/clear-text-logging",
    desc: "Sensitive data is written to logs in clear text.",
  },
  {
    rule: "Path injection",
    ruleId: "py/path-injection",
    desc: "A file path is constructed from untrusted input.",
  },
];
...
export const OAUTH_APPS: { name: string; kind: "oauth" | "github-app"; perms: string[] }[] = [
  {
    name: "Vercel",
    kind: "github-app",
    perms: ["contents:read", "deployments:write", "checks:write"],
  },
  ...
  {
    name: "Personal Script (deploy)",
    kind: "oauth",
    perms: ["repo", "workflow", "admin:repo_hook"],
  },
];
