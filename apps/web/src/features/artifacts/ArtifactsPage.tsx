...
export function ArtifactsPage() {
  ...
  const totalStorageMb =
    ...
    packages.reduce((s, p) => s + p.sizeMb, 0);
  const u = ds.usage;
  ...
  const currentRows: { id: string; sizeMb: number }[] =
    tab === "artifacts" ? artifacts : tab === "caches" ? caches : [];
  ...
  const selectedSize = currentRows
    .filter((r) => selected.has(r.id))
    .reduce((s, r) => s + r.sizeMb, 0);

  function deleteSelected() {
    const count = selected.size;
    setDeleted((d) => new Set([...d, ...selected]));
    addToast({
      title: `Deleted ${count} ${tab === "caches" ? "cache" : "artifact"}${count > 1 ? "s" : ""}`,
      description: `${fromMb(selectedSize)} reclaimed`,
      variant: "success",
    });
    setSelected(new Set());
  }
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <div className="px-2 pb-2">
            <CostAreaChart data={u.series} height={180} />
          </div>
        ...
        <Card>
          ...
          <div className="flex items-center justify-around gap-2 p-4 pt-0">
            <Gauge
              label="Minutes"
              value={u.actionsMinutes / u.minutesIncluded}
              caption={`${percent(u.actionsMinutes / u.minutesIncluded)} used`}
            />
            <Gauge
              label="Storage"
              value={u.storageGb / u.storageIncludedGb}
              caption={`${u.storageGb.toFixed(1)} / ${u.storageIncludedGb} GB`}
            />
          </div>
        </Card>
      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {(tab === "artifacts" || tab === "caches") && selected.size > 0 && (
              <Button variant="danger" size="sm" onClick={deleteSelected}>
                <Trash2 className="size-3.5" /> Delete {selected.size} · reclaim{" "}
                {fromMb(selectedSize)}
              </Button>
            )}
          </div>
  ...
  return (
    <DataTable
      ...
      empty={{
        icon: Database,
        title: "No artifacts",
        description: "Everything's been cleaned up.",
      }}
    />
  );
}
...
function PackageTable({ rows }: { rows: PackageEntry[] }) {
  ...
  const columns: Column<PackageEntry>[] = [
      ...
      cell: (r) => (
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <p className="truncate font-mono text-[0.8125rem] text-ink">{r.name}</p>
            <p className="truncate text-caption text-ink-tertiary">
              {r.registry} · {r.versions} versions
            </p>
          </div>
        </div>
      ),
    ...
    {
      id: "untagged",
      header: "Untagged layers",
      sortValue: (r) => r.untaggedLayers,
      align: "right",
      hideBelow: "sm",
      cell: (r) =>
        r.untaggedLayers > 0 ? (
          <Badge tone="warning">{r.untaggedLayers}</Badge>
        ) : (
      ...
      cell: (r) =>
        r.untaggedLayers > 0 ? (
          <Button
            ...
            onClick={() =>
              addToast({
                title: "Pruned untagged layers",
                description: `${r.name} · ${fromMb(r.sizeMb * 0.2)} reclaimed`,
                variant: "success",
              })
            }
          >
  ...
  return (
    <DataTable
      ...
      rows={rows}
      getRowId={(r) => r.id}
      empty={{ icon: Package, title: "No packages", description: "No published packages." }}
    />
  );
}
