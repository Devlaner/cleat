...
import {
  severityBreakdown, totalOpenFindings, topRiskRepos, postureDelta, membersWithout2fa,
} from "@/data/metrics";
...
export function OverviewPreview({ ds }: { ds: Dataset }) {
  ...
  const criticalEvents = ds.events.filter((e) => e.severity === "critical" || e.severity === "high").slice(0, 4);
    ...
    <div className="space-y-5 bg-canvas p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-body-sm text-ink-subtle">Security posture, reclaimable spend and critical events across your account and organizations.</p>
        </div>
        <LiveIndicator>Last scan {relativeTime(ds.events[0]?.createdAt ?? new Date().toISOString())}</LiveIndicator>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-4 gap-4">
        <StatTile label="Security posture" value={<span className={GRADE_COLOR[grade].text}>{ds.account.postureScore}</span>} icon={<ShieldCheck className="size-4" />} delta={delta} goodDirection="up" hint={<>Grade <span className={cn("font-semibold", GRADE_COLOR[grade].text)}>{grade}</span></>} accent="#5e6ad2" />
        <StatTile label="Open findings" value={findings} icon={<AlertTriangle className="size-4" />} hint={<><span className="text-critical">{sev.critical} critical</span> · {sev.high} high</>} goodDirection="down" />
        <StatTile label="Reclaimable spend" value={currency(ds.account.reclaimable)} icon={<PiggyBank className="size-4" />} hint={`${fromMb(reclaimableMb(ds))} of storage`} accent="#27a644" />
        <StatTile label="Repos at risk" value={atRisk} icon={<FolderGit2 className="size-4" />} hint={`of ${pluralize(ds.repos.length, "repo")}`} goodDirection="down" />
      </div>

      {/* trend + donut */}
      <div className="grid grid-cols-3 gap-4">
        <Card className="col-span-2">
          <CardHeader title="Security posture trend" description="Composite score over the last 30 days" action={<div className={cn("flex size-9 items-center justify-center rounded-lg text-card-title font-semibold ring-1 ring-inset", GRADE_COLOR[grade].text, GRADE_COLOR[grade].ring)}>{grade}</div>} />
          <div className="px-2 pb-3"><PostureTrendChart data={ds.postureTrend} /></div>
        </Card>
        <Card>
          ...
          <div className="flex flex-col items-center gap-4 p-4 pt-0">
            ...
            <div className="w-full"><SeverityLegend counts={sev} /></div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="col-span-2">
          <CardHeader title="Spend & reclaimable storage" description="Estimated monthly GitHub spend" action={<Badge tone="success" dot>{currency(ds.account.reclaimable)} reclaimable</Badge>} />
          <div className="px-2 pb-3"><CostAreaChart data={ds.usage.series} /></div>
        </Card>
        <Card>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          <CardHeader title="Repositories at highest risk" action={<span className="flex items-center gap-1 text-caption text-ink-subtle">View all <ArrowRight className="size-3" /></span>} />
              ...
              return (
                <div key={r.id} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-caption text-ink-tertiary">{r.openVulns} vulns · {r.openSecrets} secrets · {r.openCodeAlerts} alerts</p>
                  </div>
                  <ScoreBar value={r.hygieneScore / 100} hex={GRADE_COLOR[g].hex} className="w-20" />
        ...
        <Card>
          <CardHeader title="Recent critical events" action={<span className="flex items-center gap-1 text-caption text-ink-subtle">View all <ArrowRight className="size-3" /></span>} />
          <div className="divide-y divide-hairline border-t border-hairline">
            {criticalEvents.map((e) => {
              ...
              return (
                <div key={e.id} className="flex items-start gap-3 px-4 py-2.5">
                  <span className={cn("mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-lg", s.badge)}><Icon className="size-3.5" /></span>
                  <div className="min-w-0 flex-1">
