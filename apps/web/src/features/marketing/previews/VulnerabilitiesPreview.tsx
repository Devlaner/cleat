...
export function VulnerabilitiesPreview({ ds }: { ds: Dataset }) {
  const rows = [...ds.vulnerabilities].sort((a, b) => vulnPriority(b) - vulnPriority(a)).slice(0, 9);
  ...
  const columns: Column<Vulnerability>[] = [
    ...
    { id: "cvss", header: "CVSS", cell: (r) => <span className={cn("font-semibold tabular-nums", SEVERITY[cvssToSeverity(r.cvss)].text)}>{r.cvss.toFixed(1)}</span> },
    {
      id: "epss",
      header: "EPSS",
      className: "w-28",
    ...
    },
    {
      ...
      header: "Status",
      cell: (r) =>
        r.kev ? <Badge tone="danger"><Flame className="size-3" /> Exploited</Badge>
          : r.reachable === "reachable" ? <span className="inline-flex items-center gap-1 text-caption text-high"><Crosshair className="size-3" /> Reachable</span>
    ...
    },
    {
      ...
      header: "Fix",
      align: "right",
      cell: (r) => r.hasFixPr ? <span className="inline-flex items-center gap-1 text-caption text-primary-hover"><GitPullRequestArrow className="size-3" /> PR ready</span> : r.fixedVersion ? <span className="font-mono text-caption text-ink-muted">{r.fixedVersion}</span> : <span className="text-caption text-ink-tertiary">none</span>,
    },
  ];

  return (
    <div className="space-y-5 bg-canvas p-6">
      <PreviewHeader eyebrow="Security" title="Vulnerabilities" description="Vulnerable dependencies ranked by a fix-first priority blending severity, EPSS, KEV status and reachability." />
      <SummaryStats
        items={[
          ...
          { label: "Actively exploited", value: kev, tone: kev ? "text-critical" : undefined, icon: <Flame className="size-3.5" /> },
          { label: "With a fix available", value: fixable, tone: "text-success", icon: <GitPullRequestArrow className="size-3.5" /> },
        ]}
      />
      <PreviewFilterBar facets={["Severity", "Ecosystem", "Reachability", "Exploited (KEV)"]} count={rows.length} noun="advisories" />
