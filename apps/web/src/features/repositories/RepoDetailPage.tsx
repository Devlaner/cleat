...
export function RepoDetailPage() {
  ...
  const files = [
    ...
    { label: "CI workflow", ok: repo.hasCI, icon: Workflow },
    {
      ...
      ok: repo.branchProtected,
      icon: repo.branchProtected ? ShieldCheck : ShieldX,
    },
  ];
    ...
    <div className="space-y-5">
      ...
      <PageHeader
        ...
        actions={
          <div
            className={cn(
              "flex size-12 flex-col items-center justify-center rounded-xl text-headline font-semibold ring-1 ring-inset",
      ...
      <SummaryStats
        items={[
          { label: "Hygiene score", value: repo.hygieneScore, tone: GRADE_COLOR[grade].text },
          {
            ...
            value: vulns.length,
            tone: vulns.length ? "text-high" : "text-success",
            icon: <ShieldAlert className="size-3.5" />,
          },
          {
            label: "Secrets",
            value: secrets.length,
            tone: secrets.some((s) => s.validity === "active") ? "text-critical" : undefined,
            icon: <KeyRound className="size-3.5" />,
          },
          {
            label: "Stale branches",
            value: repo.staleBranches,
            tone: repo.staleBranches > 6 ? "text-high" : undefined,
              ...
              {files.map((f) => (
                <div
                  ...
                  className={cn(
                    ...
                    f.ok ? "border-hairline bg-surface-2" : "border-high/25 bg-high/[0.06]",
                  )}
                >
                  <span
                    className={cn(
                      "flex size-6 shrink-0 items-center justify-center rounded-md",
                      f.ok ? "bg-success/15 text-success" : "bg-high/15 text-high",
                    )}
                  >
      ...
      <Card>
        <CardHeader
          title="Open findings"
          description="Security issues detected in this repository"
        />
        <div className="border-t border-hairline">
          {vulns.length === 0 && secrets.length === 0 && codeAlerts.length === 0 ? (
            <EmptyState
              ...
              description="This repository is clean."
            />
          ) : (
            <div className="divide-y divide-hairline">
              {secrets.map((s) => (
                <FindingRow
                  ...
                  title={s.secretType}
                  meta={`${s.file}:${s.line}`}
                  tag={s.validity === "active" ? "Active" : "Revoked"}
                  tagTone={s.validity === "active" ? "text-critical" : "text-ink-tertiary"}
                />
              ))}
              {vulns.map((v) => (
                <FindingRow
                  ...
                  severity={v.severity}
                  title={v.title}
                  meta={`${v.package} ${v.currentVersion}`}
                  tag={`CVSS ${v.cvss.toFixed(1)}`}
                  tagTone={SEVERITY[v.severity].text}
                />
              ))}
              {codeAlerts.map((a) => (
                <FindingRow
...
function BackLink() {
  return (
    <Link
      to="/app/repositories"
      className="inline-flex items-center gap-1.5 text-body-sm text-ink-subtle transition-colors hover:text-ink"
    >
    ...
    <div>
      <div className="mb-1.5 flex items-center justify-between gap-2">
        ...
        <span
          ...
          style={{ color: scoreHex(check.score) }}
        >
          {check.score}/10
        </span>
      </div>
