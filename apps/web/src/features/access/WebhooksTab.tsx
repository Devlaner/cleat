...
import type { Webhook } from "@cleat/contracts";
...
export function WebhooksTab({ webhooks }: { webhooks: Webhook[] }) {
  const facets: FacetDef<Webhook>[] = [
    ...
    },
    {
      key: "health",
      label: "Health",
      accessor: (r) => (r.lastStatus >= 200 && r.lastStatus < 300 ? "ok" : "failing"),
      options: [
        { value: "ok", label: "Delivering" },
        { value: "failing", label: "Failing" },
      ],
    },
  ];

  const rows = useFilteredRows(TABLE, webhooks, {
    search: (r) => `${r.url} ${r.scope} ${r.events.join(" ")}`,
    facets,
  });
  ...
  const columns: Column<Webhook>[] = [
    ...
    },
    {
      ...
      hideBelow: "lg",
      cell: (r) => (
        <span className="text-caption text-ink-subtle">{relativeTime(r.lastDelivery)}</span>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        rows={rows}
        getRowId={(r) => r.id}
        empty={{ icon: WebhookIcon, title: "No webhooks", description: "No webhooks configured." }}
      />
    </div>
  );
}
