...
const STATUS_META: Record<CodeScanAlert["status"], { label: string; tone: "warning" | "success" | "muted"; icon: typeof CircleDot }> = {
...
export function CodeScanningPage() {
  ...
  const facets: FacetDef<CodeScanAlert>[] = [
    ...
    { key: "status", label: "Status", accessor: (r) => r.status, options: [{ value: "open", label: "Open" }, { value: "fixed", label: "Fixed" }, { value: "dismissed", label: "Dismissed" }] },
    { key: "tool", label: "Tool", accessor: (r) => r.tool, options: [...new Set(ds.codeAlerts.map((a) => a.tool))].map((t) => ({ value: t, label: t })) },
  ];
  ...
  const columns: Column<CodeScanAlert>[] = [
    ...
    { id: "severity", header: "Severity", sortValue: (r) => SEVERITY[r.severity].rank, cell: (r) => <SeverityBadge severity={r.severity} /> },
    { id: "repo", header: "Repository", sortValue: (r) => r.repo, hideBelow: "md", cell: (r) => <span className="font-mono text-[0.8125rem] text-ink-muted">{r.repo}</span> },
    { id: "file", header: "Location", hideBelow: "lg", cell: (r) => <span className="font-mono text-[0.8125rem] text-ink-subtle">{r.file}:{r.line}</span> },
    { id: "tool", header: "Tool", sortValue: (r) => r.tool, hideBelow: "lg", cell: (r) => <Badge tone="neutral">{r.tool}</Badge> },
    {
      id: "status",
      header: "Status",
    ...
    { id: "detectedAt", header: "Detected", sortValue: (r) => r.detectedAt, align: "right", hideBelow: "sm", cell: (r) => <span className="text-ink-subtle">{relativeTime(r.detectedAt)}</span> },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <SummaryStats
        items={[
          { label: "Open alerts", value: open, tone: open > 0 ? "text-high" : "text-success", icon: <CircleDot className="size-3.5" /> },
          { label: "Fixed", value: fixed, tone: "text-success", icon: <CircleCheck className="size-3.5" /> },
      ...
      <DataTable
        ...
        rows={rows}
        getRowId={(r) => r.id}
        empty={{ icon: ShieldCheck, title: "No code-scanning alerts", description: "Nothing matches the current filters." }}
      />
    </div>
  );
}
