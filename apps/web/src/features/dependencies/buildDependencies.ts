...
const LICENSES: [string, number][] = [
  ["MIT", 10], ["Apache-2.0", 6], ["BSD-3-Clause", 4], ["ISC", 3],
  ["MPL-2.0", 2], ["LGPL-3.0", 1], ["GPL-3.0", 1], ["AGPL-3.0", 1], ["Unlicensed", 1],
];
...
export function buildDependencies(ds: Dataset): Dependency[] {
  ...
  for (const [ecosystem, names] of Object.entries(PACKAGES)) {
    for (const name of names) {
      ...
      const latestVersion = outdated ? `${major + r.int(0, 2)}.${r.int(0, 9)}.${r.int(0, 30)}` : version;
  ...
  return out.sort((a, b) => Number(b.vulnerable) - Number(a.vulnerable) || a.name.localeCompare(b.name));
}
...
export function licenseDistribution(deps: Dependency[]) {
  ...
  const palette = ["#5e6ad2", "#828fff", "#5b9bf0", "#27a644", "#ecc24a", "#7a7fad", "#ff9352", "#fb5b78", "#62666d"];
  return [...map.entries()]
    .sort((a, b) => b[1] - a[1])
    .map(([license, count], i) => ({ license, count, hex: palette[i % palette.length]!, copyleft: COPYLEFT.has(license) }));
}
