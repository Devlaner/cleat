...
import type { PostureTrendPoint } from "@/data/types";
import { shortDate } from "@/lib/format";
import { ChartTooltipBox } from "./ChartTooltip";

export function PostureTrendChart({ data, height = 200 }: { data: PostureTrendPoint[]; height?: number }) {
          ...
          content={({ active, payload }) =>
            active && payload?.length ? (
              <ChartTooltipBox
                ...
                rows={[{ label: "Posture score", value: payload[0]!.value as number, hex: "#5e6ad2" }]}
              />
            ) : null
          }
        />
        <Area type="monotone" dataKey="score" stroke="#5e6ad2" strokeWidth={2} fill="url(#posture-grad)" isAnimationActive={!reduce} />
      </AreaChart>
    </ResponsiveContainer>
    </div>
  );
