  ...
  return (
    ...
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 6, right: 6, bottom: 0, left: -10 }}>
        ...
        <XAxis dataKey="label" tick={{ fill: "#62666d", fontSize: 11 }} tickLine={false} axisLine={false} minTickGap={16} />
        <YAxis tickFormatter={(v) => `$${v}`} tick={{ fill: "#62666d", fontSize: 11 }} tickLine={false} axisLine={false} width={48} />
        <Tooltip
          ...
          content={({ active, payload }) =>
            active && payload?.length ? (
              <ChartTooltipBox
                ...
                rows={[{ label: "Spend", value: currency(payload[0]!.value as number), hex: "#828fff" }]}
              />
            ) : null
          }
        />
        <Area type="monotone" dataKey="cost" stroke="#828fff" strokeWidth={2} fill="url(#cost-grad)" isAnimationActive={!reduce} />
      </AreaChart>
    </ResponsiveContainer>
    </div>
  );
